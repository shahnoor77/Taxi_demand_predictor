...
def find_best_hyperparameters(
    ...
    def objective(trial: optuna.trial.Trial) -> float:
        ...
        for train_index, val_index in tss.split(X_train):
            ...
            X_train_, X_val_ = X_train.iloc[train_index,
...
def load_features_and_target(
    ...
    if local_path_features_and_target:
        ...
        try:
            ...
            logger.info(
                f'Saved features_and_target to local file at {local_file}')
        except:
...
def train(
    ...
    features_and_target = load_features_and_target(
        local_path_features_and_target)
    ...
    logger.info(
        f'Splitting data into training and test sets with {cutoff_date=}')
    ...
    if test_mae < config.MAX_MAE:
        ...
        logger.info(
            f'Model version {model_version} pushed to the model registry.')
